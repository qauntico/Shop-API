    ...
    form.parse(req, async (err, fields, files) => {
      ...
      if (!fields.name) {
        return res.status(400).json({
          error:
            "There is No Event Name How Will People Identify What Event It is",
        });
      }
      if (!fields.description) {
        return res
          .status(400)
          .json({ error: "Describe Your Event So That People Can Know" });
      }
      if (!fields.category) {
        return res
          .status(400)
          .json({ error: "The Category For Your Event Was not Selected" });
      }
      if (!fields.startTime) {
        return res
          .status(400)
          .json({ error: "At What Time Does Your Event Start" });
      }
      if (!fields.startDate) {
        return res
          .status(400)
          .json({ error: "What Is The Start Date For Your Event" });
      }
      if (!fields.location) {
        return res
          .status(400)
          .json({ error: "Location For This Event ? ....." });
      }
      if (!files.photo) {
      ...
      if (result) {
        return res.json({success: "Event Was Succesfully Created"})
      }
     ...
     return res.status(403).json({error: "could not save data an errored occured"})
  }
};
...
exports.update = async (req, res) => {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    await product
      .save()
      .then((result) =>
        res.json({ success: "Event Was Successfully Updated.." })
      )
      .catch((err) =>
        res.status(403).json({
          error: "Could Not Update Event An Error Occurred Try Again Later",
        })
      );
  });
};
...
exports.remove = async (req, res) => {
  await Product.findOneAndDelete({ name: req.product.name })
    ...
    .catch((err) => {
      res
        .status(403)
        .json({ error: "Could Not Delete Product And Error Occured" });
    });
};
